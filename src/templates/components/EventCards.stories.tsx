...
export const EventCardComponent: Story = {
    name: 'Event Cards',
    parameters: {
        a11y: {
            config: {
    ...
    render: () => (
        <>
            ...
            <Main>
                ...
                <Column cols="3">
                    {EventData.slice(0, 6).map(item => <Card key={`event-${item.id}`} noImage={!item.image}>
                ...
                <Column cols="4" maxWidth="alignwide">
                    {EventData.slice(0, 8).map(item => <Card key={`event-${item.id}`} noImage={!item.image}>
                        {item.image && <Card.Figure>
                                <img src={item.image} alt={item.alt} width="600" height="400" />
                            </Card.Figure>}
                        <Card.DateThumb startDate={item.startDate} endDate={item.endDate} />
