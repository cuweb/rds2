...
This section is for Carleton developers using `@cuweb/rds2` in their projects.
...
```tsx
import { Button, Icon } from '@cuweb/rds2';
...
`@cuweb/rds2` ships with `@cuweb/rds-icons` as a peer dependency. Both packages must be installed. Authentication for GitHub Packages is required because rds-icons is private by design — see [icons.md](icons.md) for the reason.
...
```sh
pnpm up @cuweb/rds2 @cuweb/rds-icons
```
...
```sh
pnpm remove @cuweb/rds2 @cuweb/rds-icons
```

Also remove the `@cuweb:registry=...` line from `.npmrc` (unless other `@cuweb/*` packages are in use) and any `import '@cuweb/rds2/styles'` references.
